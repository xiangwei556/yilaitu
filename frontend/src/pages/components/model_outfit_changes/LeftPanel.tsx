...
export const LeftPanel: React.FC<LeftPanelProps> = ({ 
  ...
  const [selectedOutfitType, setSelectedOutfitType] = useState<'single' | 'top-bottom'>('single');
  ...
  const singleOutfitInputRef = useRef<HTMLInputElement>(null);
  const topOutfitInputRef = useRef<HTMLInputElement>(null);
  const bottomOutfitInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleSingleOutfitUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setSingleOutfitImage(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };

  const handleTopOutfitUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setTopOutfitImage(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };

  const handleBottomOutfitUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setBottomOutfitImage(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSingleOutfitClick = () => {
    singleOutfitInputRef.current?.click();
  };

  const handleTopOutfitClick = () => {
    topOutfitInputRef.current?.click();
  };

  const handleBottomOutfitClick = () => {
    bottomOutfitInputRef.current?.click();
  };
  ...
  const handleGenerate = async () => {
    ...
    if (selectedOutfitType === 'single') {
      if (!singleOutfitImage) {
        setValidationMessage('请先上传服装图');
      ...
      if (!topOutfitImage) {
        setValidationMessage('请先上传上装图');
        setShowValidationOverlay(true);
        setTimeout(() => setShowValidationOverlay(false), 2000);
        return;
      }
      if (!bottomOutfitImage) {
        setValidationMessage('请先上传下装图');
        setShowValidationOverlay(true);
        setTimeout(() => setShowValidationOverlay(false), 2000);
        return;
      }
    ...
    console.log('=== 用户选择的内容 ===');
    console.log('服装类型:', selectedOutfitType === 'single' ? '单件' : '上下装');
    if (selectedOutfitType === 'single') {
      console.log('上传的服装图:', singleOutfitImage);
    } else {
      console.log('上传的上装图:', topOutfitImage);
      console.log('上传的下装图:', bottomOutfitImage);
    }
    ...
    <div className="relative w-[380px] bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full">
      ...
      <header className="sticky top-0 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm px-4 pt-4 pb-3 border-b border-gray-200 dark:border-gray-700">
        <div className="flex justify-between items-center h-11">
          <div className="flex items-center gap-1">
            <h1 className="text-lg font-semibold">模特换装</h1>
          </div>
      ...
      <main className="px-4 py-6 space-y-6 pb-28 overflow-y-auto flex-1 custom-scrollbar">
        <div className="space-y-4">
          <div className="bg-slate-100 dark:bg-surface-dark p-1 rounded-full flex text-sm">
            <button 
              onClick={() => setSelectedOutfitType('single')}
              className={clsx(
                'flex-1 text-center py-1.5 rounded-full',
                selectedOutfitType === 'single'
                  ? 'bg-surface-light dark:bg-background-dark text-primary font-medium shadow-sm'
            ...
            <button 
              onClick={() => setSelectedOutfitType('top-bottom')}
              className={clsx(
                'flex-1 text-center py-1.5 rounded-full',
                selectedOutfitType === 'top-bottom'
                  ? 'bg-surface-light dark:bg-background-dark text-primary font-medium shadow-sm'
                  : 'text-text-secondary-light dark:text-text-secondary-dark'
              )}
            >
          ...
          {selectedOutfitType === 'single' ? (
            <>
              <input
                ref={singleOutfitInputRef}
                type="file"
                accept="image/*"
                onChange={handleSingleOutfitUpload}
                className="hidden"
              />
              {!singleOutfitImage ? (
                <div 
                  onClick={handleSingleOutfitClick}
                  className="bg-primary/5 dark:bg-primary/10 border border-dashed border-primary/30 dark:border-primary/50 rounded-xl p-6 text-center flex flex-col items-center justify-center h-32 cursor-pointer hover:bg-primary/10 transition-colors"
                >
                  <div className="flex justify-center mb-3">
                    <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
                      <span className="material-symbols-outlined text-3xl text-primary">apparel</span>
                    </div>
                  </div>
                  <p className="font-medium text-text-primary-light dark:text-text-primary-dark">上传服装</p>
                  <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark mt-1">支持JPG/JPEG/PNG格式</p>
                </div>
              ) : (
                <div className="bg-primary/5 dark:bg-primary/10 border border-dashed border-primary/30 dark:border-primary/50 rounded-xl p-3 flex items-center h-32 gap-4">
                  <div className="h-full aspect-[3/4] relative rounded-lg overflow-hidden bg-white border border-black/5 flex-shrink-0">
                    <img src={singleOutfitImage} alt="Uploaded outfit" className="w-full h-full object-cover" />
                  </div>
            ...
            <>
              <input
                ref={topOutfitInputRef}
                type="file"
                accept="image/*"
                onChange={handleTopOutfitUpload}
                className="hidden"
              />
              {!topOutfitImage ? (
                <div 
                  onClick={handleTopOutfitClick}
                  className="bg-primary/5 dark:bg-primary/10 border border-dashed border-primary/30 dark:border-primary/50 rounded-xl p-6 text-center flex flex-col items-center justify-center h-32 cursor-pointer hover:bg-primary/10 transition-colors"
                >
                  <div className="flex justify-center mb-3">
                    <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
                  ...
                  <p className="font-medium text-text-primary-light dark:text-text-primary-dark">上传上装</p>
                  <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark mt-1">支持JPG/JPEG/PNG格式</p>
                </div>
              ) : (
                <div className="bg-primary/5 dark:bg-primary/10 border border-dashed border-primary/30 dark:border-primary/50 rounded-xl p-3 flex items-center h-32 gap-4">
                  <div className="h-full aspect-[3/4] relative rounded-lg overflow-hidden bg-white border border-black/5 flex-shrink-0">
                    <img src={topOutfitImage} alt="Uploaded top outfit" className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 flex justify-center items-center">
                    <button 
                      onClick={handleTopOutfitClick}
              ...
              <input
                ref={bottomOutfitInputRef}
                type="file"
                accept="image/*"
                onChange={handleBottomOutfitUpload}
                className="hidden"
              />
              {!bottomOutfitImage ? (
                <div 
                  onClick={handleBottomOutfitClick}
                  className="bg-primary/5 dark:bg-primary/10 border border-dashed border-primary/30 dark:border-primary/50 rounded-xl p-6 text-center flex flex-col items-center justify-center h-32 cursor-pointer hover:bg-primary/10 transition-colors"
                >
                  <div className="flex justify-center mb-3">
                    <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
                      <span className="material-symbols-outlined text-3xl text-primary">woman</span>
                    </div>
                  </div>
                  <p className="font-medium text-text-primary-light dark:text-text-primary-dark">上传下装</p>
                  <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark mt-1">支持JPG/JPEG/PNG格式</p>
                </div>
              ) : (
                <div className="bg-primary/5 dark:bg-primary/10 border border-dashed border-primary/30 dark:border-primary/50 rounded-xl p-3 flex items-center h-32 gap-4">
                  <div className="h-full aspect-[3/4] relative rounded-lg overflow-hidden bg-white border border-black/5 flex-shrink-0">
                    <img src={bottomOutfitImage} alt="Uploaded bottom outfit" className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 flex justify-center items-center">
                    <button 
                      onClick={handleBottomOutfitClick}
        ...
        >
          ...
          <div className="bg-white rounded-2xl w-[300px] h-full max-h-[calc(100vh-56px)] flex flex-col shadow-2xl relative z-10 border border-gray-100 overflow-hidden hover-scroll">
            <div className="flex items-center justify-between p-4 border-b border-gray-50">
              <h3 className="font-bold text-base text-gray-800">{selectedModelTab === 'my' ? '我的参考图' : '推荐参考图'}</h3>
